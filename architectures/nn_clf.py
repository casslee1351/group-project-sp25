...
class BaseRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # x: (batch_size, 1, input_dim) or (batch_size, max_seq_len, input_dim)
        if len(x.shape) == 2:
        ...
        out = self.fc(hidden)
        # out = self.softmax(out) # softmax handled by CrossEntropyLoss in training
...
class DeepTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # x: (batch_size, 1, 768) or (batch_size, max_seq_len, 768)
        if x.dim() == 2:
            x = x.unsqueeze(1)
        ...
        # remove sequence dimension (either directly or by taking mean)
        # x: (batch_size, 768)
        if x.dim() == 2:
            # likely from the DistilBERT embedding
            x = x.squeeze(1)
        elif x.dim() == 3:
            # likely from the GloVe embedding
            x = x.mean(dim = 1)
